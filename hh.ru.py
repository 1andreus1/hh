...
area='4228'
try:
    with open('Академия нефти и газа.txt','r',encoding='utf-8') as f:
        txt=f.readlines()
        area=txt[0][:-1]
...
def get_resumes(id_area):
    ...
    while 1:
        try:
            sleep(t)
            pages=int(requests.get('https://api.hh.ru/resumes', headers=headers, params={'educational_institution':'39171','area': id_area}).json()['pages'])
            break
        except:
            print('Неудача')
            if t == 0:
    ...
    for p in range(pages):
        ...
        while 1:
            try:
                sleep(t)
                res = requests.get('https://api.hh.ru/resumes',headers=headers,params={'educational_institution':'39171','area': id_area,'per_page':100,'page':p})
                break
            except:
                print('Неудача')
                if t == 0:
...
for id_area in get_cities():
    ...
    for i in resumes:
        s += str(k) + ' ' + i + '\n'
        k += 1
    with open('Академия нефти и газа.txt', 'w+', encoding='utf-8') as f:
        f.write(s)
    print(sum)
